...
public class VisitService : IVisitService
{
    ...
    public async Task<MessageResponse> Create(CreateVisitRequestDto request)
    {
        ...
        if (Math.Abs(visitTimeSpan.TotalMinutes) <= 60)
        {
            ...
            if (IsFree(data, subscription))
            {
                data.Price = 0;
                subscription.MonthlyVisits--;
            }else
            {
    ...
    private static bool IsFree(Visit x, Subscription sub)
    {
        if(x.TimeShift.Name.Contains("While Work")||x.TimeShift.Name.Contains("ساعات الدوام"))
        {
            if (sub.MonthlyVisits > 0)
                return true;
        }
        return false;
    }
