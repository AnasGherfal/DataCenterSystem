...
function getdata() {
  ...
  subscriptionApi
    ...
    .then(function (response) {
        subs.value = response.data.content;
    //   subs.id = content.id;
    ...
    //   subs.totalPrice = content.totalPrice;
      subs.value.daysRemaining = moment(subs.value.endDate).diff(moment(), "days");
    })
    // .then(function () {
    ...
    <template #content>
      ...
      <div class="grid p-fluid">
        ...
        <div v-else class="flex-1" style="text-align: center; width: 20rem">
          ...
          <div>
            ...
            <div v-else-if="subs.daysRemaining <= 0 && subs.status !== 2">
              ...
              <h5 class="text-red-600" style="margin-bottom: 5px">
                اضغط على الأيقونة لطلب للتجديد
              </h5>
              <Button
                icon="fa-solid fa-repeat"
                severity="danger"
            ...
            <div v-else="subs.daysRemaining <= 0 && subs.status !== 2">
              <h3 class="text-red-600">هذا الاشتراك انتهى</h3>

              <!-- <h5 class="text-red-600" style="margin-bottom: 5px">
                اضغط على الأيقونة لطلب التجديد
              </h5>
              <Button
                icon="fa-solid fa-repeat"
                severity="danger"
        ...
        <div class="flex-1">
          ...
          <Skeleton v-if="loading" width="50%" height="1rem"></Skeleton>
          <div
            v-for="(item, index) in subs.files"
            :key="index"
