...
public sealed record RequestNewVisitCommandHandler : IRequestHandler<RequestNewVisitCommand, MessageResponse>
{
    private readonly IClientService _client;
    private readonly AppDbContext _dbContext;

    public RequestNewVisitCommandHandler(AppDbContext dbContext, IClientService client)
    {
        _dbContext = dbContext;
        _client = client;
    }
    
    public async Task<MessageResponse> Handle(RequestNewVisitCommand request, CancellationToken cancellationToken)
    {
        ...
        var @event = new VisitRequestedEvent(_client.GetIdentifier(), Guid.NewGuid(), new VisitRequestedEventData()
        { 
            ...
            Companions = request.Companions.Select(p => new VisitRequestedEventDataExtra()
            {
