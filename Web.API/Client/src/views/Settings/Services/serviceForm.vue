    ...
    <div class="grid p-fluid">
      ...
      <div class="field col-12 md:col-4">
        <span class="p-float-label">
          <InputText
            id="amountOfPower"
            type="text"
            v-model="service.amountOfPower"
          />
          <label for="amountOfPower">Amount Of Power</label>
          <div style="height: 2px">
            <error
              v-for="error in v$.amountOfPower.$errors"
              :key="error.$uid"
              class="p-error"
              >{{ error.$message }}</error
            >
      ...
      <div class="field col-12 md:col-4">
        <span class="p-float-label">
          ...
          <div style="height: 2px">
            <error
              v-for="error in v$.acpPort.$errors"
              :key="error.$uid"
              class="p-error"
              >{{ error.$message }}</error
            >
      ...
      <div class="field col-12 md:col-4">
        <span class="p-float-label">
          <InputNumber
            id="monthlyVisits"
            type="text"
            v-model="service.monthlyVisits"
          />
          <label for="monthlyVisits">عدد الزيارات في الشهر</label>
          <div style="height: 2px">
            <error
              v-for="error in v$.monthlyVisits.$errors"
              :key="error.$uid"
              class="p-error"
              >{{ error.$message }}</error
            >
      ...
      <div class="field col-12 md:col-4">
        <span class="p-float-label">
          <InputNumber id="price" type="text" v-model="service.price" />
